    ...
    *,
    ...
    def write_text(
        ...
        for lrect, spans in nlines:
            ...
            # 在每一行文本后追加定位信息（可选）
            if emit_positions:
                r = lrect if lrect is not None else parms.clip
                bbox_vals = (r.x0, r.y0, r.x1, r.y1)
                bbox_str = ",".join([str(round(v, 1)) for v in bbox_vals])
                page_no = parms.page.number + 1
                out_string += f"\n<sub>pos: page={page_no}, bbox={bbox_str}</sub>\n"
        out_string += "\n"
        if code:
            out_string += "```\n"  # switch of code mode
            code = False
    ...
    def output_tables(parms, text_rect):
        ...
        if text_rect is not None:  # select tables above the text block
            for i, trect in sorted(
                ...
                if emit_positions:
                    r = pymupdf.Rect(parms.tabs[i].bbox) | pymupdf.Rect(parms.tabs[i].header.bbox)
                    bbox_str = ",".join([str(round(v, 1)) for v in (r.x0, r.y0, r.x1, r.y1)])
                    page_no = parms.page.number + 1
                    this_md += f"<sub>pos: page={page_no}, bbox={bbox_str}</sub>\n"
                if EXTRACT_WORDS:
                    # for "words" extraction, add table cells as line rects
                    cells = sorted(
                        set(
