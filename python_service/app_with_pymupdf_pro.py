...
# 导入配置
from config import ES_CONFIG, DOCUMENT_CONFIG, EMBEDDING_CONFIG, RAG_CONFIG, PYMUPDF_PRO_CONFIG
...
try:
    import pymupdf.pro
    pymupdf.pro.unlock(PYMUPDF_PRO_CONFIG['trial_key'])
    PYMUPDF_PRO_AVAILABLE = True
    logger.info("✅ PyMuPDF Pro 初始化成功")
except Exception as e:
    logger.warning(f"⚠️  PyMuPDF Pro 不可用: {e}")
    logger.info("将使用传统方案作为回退")
...
try:
    ...
    logger.info("✅ PyMuPDF4LLM 可用")
except ImportError:
    PYMUPDF4LLM_AVAILABLE = False
    logger.warning("⚠️  PyMuPDF4LLM 不可用")
...
def read_root():
    return {
        "message": "Knowledge Base Python Service (PyMuPDF Pro)",
        "pymupdf_pro_available": PYMUPDF_PRO_AVAILABLE,
...
def process_document_with_pymupdf_pro(file_path: str, knowledge_id: int, knowledge_name: str, 
                                   description: str, tags: str, effective_time: str):
    """
    使用 PyMuPDF Pro 统一处理文档
    """
    logger.info(f"使用 PyMuPDF Pro 处理文档: {file_path}")
    ...
    try:
        ...
        # 如果 PyMuPDF4LLM 分块失败或不可用，使用传统分块
        if not chunks:
        ...
        logger.error(f"PyMuPDF Pro 文档处理失败: {e}")
        raise HTTPException(status_code=500, detail=f"文档处理失败: {str(e)}")

def process_document_traditional(file_path: str, knowledge_id: int, knowledge_name: str, 
                               description: str, tags: str, effective_time: str):
    """
    使用传统方案处理文档
    """
    logger.info(f"使用传统方案处理文档: {file_path}")
        ...
        try:
            ...
            if PYMUPDF_PRO_AVAILABLE:
                try:
                    ...
                    logger.warning(f"PyMuPDF Pro 处理失败，回退到传统方案: {e}")
...
def health_check():
    ...
    try:
        ...
        return {
            ...
            "pymupdf_pro_available": PYMUPDF_PRO_AVAILABLE,
            "pymupdf4llm_available": PYMUPDF4LLM_AVAILABLE,
            "trial_key": PYMUPDF_PRO_CONFIG['trial_key'][:10] + "...",
