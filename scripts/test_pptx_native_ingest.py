...
# -*- coding: utf-8 -*-
"""
测试脚本：验证 PPTX 原生解析与入库

功能：
- 调用 Python 服务 /api/document/process 上传一个 PPTX 文件（原生解析，不依赖外部工具）
- 成功后从 ES 检索该知识与文件对应的 chunks，打印关键信息（page_num、chunk_index、bbox、positions 数量）

Windows 运行示例：
  python scripts/test_pptx_native_ingest.py --python-url http://localhost:8000 --file python_service/file/network_skill.pptx --knowledge-id 104 --name 测试PPTX
"""
...
def main():
    ...
    parser.add_argument('--file', required=True, help='PPTX 文件路径 (.pptx)')
