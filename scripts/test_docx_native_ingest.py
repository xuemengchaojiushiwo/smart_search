...
# -*- coding: utf-8 -*-
"""
测试脚本：验证 Word(DOCX) 原生解析与入库

功能：
- 调用 Python 服务 /api/document/process 上传一个 DOCX 文件（原生解析，不依赖外部工具）
- 成功后从 ES 检索该知识与文件对应的 chunks，打印关键信息（page_num、chunk_index、bbox、positions 数量）

Windows 运行示例：
  python scripts/test_docx_native_ingest.py --python-url http://localhost:8000 --file python_service/file/manus介绍.docx --knowledge-id 103 --name 测试DOCX
"""
...
def process_document(python_url: str, file_path: str, knowledge_id: int, knowledge_name: str,
    ...
    files = {
        'file': (os.path.basename(file_path), open(file_path, 'rb'), 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')
    }
    data = {
...
def search_es(knowledge_id: int, source_file: str) -> None:
    ...
    # 1) keyword 精确
    q1 = {"size": 10, "query": {"bool": {"must": [{"term": {"knowledge_id": knowledge_id}}, {"term": {"source_file.keyword": source_file}}]}}, "_source": base_source}
    n = run_and_print(q1, "knowledge_id + source_file.keyword")
    if n > 0:
        return

    # 2) 短语匹配
    q2 = {"size": 10, "query": {"bool": {"must": [{"term": {"knowledge_id": knowledge_id}}, {"match_phrase": {"source_file": source_file}}]}}, "_source": base_source}
    n = run_and_print(q2, "knowledge_id + source_file 短语匹配")
    if n > 0:
        return

    # 3) 仅 knowledge_id
    q3 = {"size": 10, "query": {"term": {"knowledge_id": knowledge_id}}, "_source": base_source}
    run_and_print(q3, "仅 knowledge_id（排查）")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--python-url', default='http://localhost:8000')
    parser.add_argument('--file', required=True, help='DOCX 文件路径 (.docx)')
