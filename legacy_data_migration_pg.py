...
# -*- coding: utf-8 -*-
"""
历史系统数据迁移工具 - PostgreSQL 版本
从 SQLite3 迁移到 PostgreSQL
"""
...
class LegacyDataMigrationPG:
    ...
    def connect_databases(self):
        ...
        try:
            ...
            self.pg_conn.commit()
            print(f"✅ 已连接 PostgreSQL 数据库: {self.pg_config['database']}")
    ...
    def ensure_target_schema(self):
        ...
        with self.pg_conn.cursor() as cursor:
            add_columns_sql = [
                ...
                "ALTER TABLE users ADD COLUMN IF NOT EXISTS date_joined TIMESTAMP",
            ]
    ...
    def migrate_users(self):
        ...
        with self.pg_conn.cursor() as pg_cursor:
            for user in users:
                ...
                # 生成新的用户ID
                pg_cursor.execute("SELECT COALESCE(MAX(id), 0) + 1 FROM users")
    ...
    def migrate_categories(self):
        ...
        with self.pg_conn.cursor() as pg_cursor:
            ...
            for category in categories:
                ...
                # 生成新的ID
                pg_cursor.execute("SELECT COALESCE(MAX(id), 0) + 1 FROM knowledge")
                new_id = pg_cursor.fetchone()[0]
            ...
            for content in contents:
                ...
                if creator_id:
                    ...
                    if creator_result and creator_result[0]:
                        ...
                        # 在新表中查询对应的用户ID
                        pg_cursor.execute("SELECT id FROM users WHERE staffid = %s", (username,))
    ...
    def migrate_uploads(self):
        ...
        with self.pg_conn.cursor() as pg_cursor:
            for upload in uploads:
                ...
                # 生成新的ID
                pg_cursor.execute("SELECT COALESCE(MAX(id), 0) + 1 FROM attachments")
                new_id = pg_cursor.fetchone()[0]
    ...
    def migrate_feedbacks(self):
        ...
        cursor = self.sqlite_conn.cursor()
        # 获取app_feedback表的数据，检查是否有creator_id字段
        cols = self._sqlite_columns('app_feedback')
        if 'creator_id' in cols:
            cursor.execute("SELECT id, text, created, content_item_id, creator_id FROM app_feedback")
        else:
            cursor.execute("SELECT id, text, created, content_item_id, NULL as creator_id FROM app_feedback")
        ...
        with self.pg_conn.cursor() as pg_cursor:
            ...
            for feedback in feedbacks:
                ...
                # 生成新的ID
                pg_cursor.execute("SELECT COALESCE(MAX(id), 0) + 1 FROM knowledge_feedbacks")
                new_id = pg_cursor.fetchone()[0]
    ...
    def migrate_favorites(self):
        ...
        with self.pg_conn.cursor() as pg_cursor:
            ...
            for favorite in favorites:
                ...
                # 生成新的ID
                pg_cursor.execute("SELECT COALESCE(MAX(id), 0) + 1 FROM knowledge_favorites")
                new_id = pg_cursor.fetchone()[0]
                ...
                if user_id in new_user_id_cache:
                    ...
                    user_result = pg_cursor.fetchone()
                    if user_result:
                        new_user_id = user_result[0]  # 使用数字ID
                        # 将结果存入缓存
                        new_user_id_cache[user_id] = new_user_id
                        print(f"  收藏 {old_id}: 找到用户 {username}, 用户ID={new_user_id}")
                    else:
    ...
    def migrate_search_history(self):
        ...
        cursor = self.sqlite_conn.cursor()
        # 检查app_helpfulness表是否存在
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='app_helpfulness'")
        if cursor.fetchone():
            ...
            print("  app_helpfulness表不存在，跳过搜索历史迁移")
            search_histories = []
        ...
        with self.pg_conn.cursor() as pg_cursor:
            ...
            for history in search_histories:
                ...
                # 生成新的ID
                pg_cursor.execute("SELECT COALESCE(MAX(id), 0) + 1 FROM search_history")
                new_id = pg_cursor.fetchone()[0]
                ...
                if user_id in new_user_id_cache:
                    ...
                    if user_result:
                        ...
                        # 将结果存入缓存
                        new_user_id_cache[user_id] = new_user_id
                        print(f"  搜索历史 {old_id}: 找到用户 {username}, 用户ID={new_user_id}")
                    else:
    ...
    def fix_sequences(self):
        ...
        try:
            with self.pg_conn.cursor() as cursor:
                ...
                for table, sequence in table_sequences.items():
                    try:
                        # 获取表中当前最大的ID
                        ...
                        print(f"✅ {table}: 最大ID={max_id}, 序列设置为={next_val}")
                    except Exception as e:
                        print(f"⚠️ 修复表 {table} 的序列失败: {e}")
                        # 继续处理其他表
                ...
                for table, sequence in table_sequences.items():
                    try:
                        ...
                        print(f"  {sequence}: {current_seq_val}")
                    except Exception as e:
                        print(f"⚠️ 获取序列 {sequence} 状态失败: {e}")
    ...
    def clear_target_tables(self):
        ...
        with self.pg_conn.cursor() as cursor:
            # 按依赖关系逆序删除
            tables = [
                'knowledge_favorites', 'knowledge_feedbacks', 'search_history',
                'attachments', 'knowledge_versions', 'knowledge', 'users'
            ]
            for table in tables:
                try:
...
def main():
    # 配置 - 使用默认值进行测试
    sqlite_db_path = "test_legacy.db"  # 使用现有的测试数据库
    ...
    print(f"🐘 PostgreSQL数据库: {pg_config['host']}:{pg_config['port']}/{pg_config['database']}")
    
    # 运行迁移
    migration = LegacyDataMigrationPG(sqlite_db_path, pg_config)
