...
class LegacyDataMigration:
    ...
    def _parse_expire_date(self, date_str, ctx: str):
        """解析过期日期字符串，支持多种格式"""
        if not date_str:
            return None
        ...
        # 支持的日期格式
        date_formats = [
            '%Y年%m月%d日 上午%H:%M:%S',  # 2025年11月6日 上午12:00:00
            '%Y年%m月%d日 下午%H:%M:%S',  # 2025年11月6日 下午12:00:00
            '%Y年%m月%d日 %H:%M:%S',      # 2025年11月6日 12:00:00
            '%Y年%m月%d日',               # 2025年11月6日
            '%Y-%m-%d %H:%M:%S',          # 2025-11-06 12:00:00
            '%Y-%m-%d',                   # 2025-11-06
            '%Y/%m/%d %H:%M:%S',          # 2025/11/06 12:00:00
            '%Y/%m/%d',                   # 2025/11/06
        ]
        ...
        # 如果所有格式都失败，尝试使用正则表达式处理中文日期
        try:
            import re
            # 匹配 "2025年11月6日 上午12:00:00" 格式
            match = re.match(r'(\d{4})年(\d{1,2})月(\d{1,2})日\s*(上午|下午)?\s*(\d{1,2}):(\d{1,2}):(\d{1,2})?', date_str)
            if match:
        ...
        logging.warning(f"日期解析失败: {ctx} 无法解析日期格式 '{date_str}'")
        return None
    ...
    def migrate_content(self):
        ...
        # 兼容历史命名：expired、expir_date 或 expire_date
        expired_expr = (
            'expired' if 'expired' in cols else (
                'expir_date AS expired' if 'expir_date' in cols else (
                    'expire_date AS expired' if 'expire_date' in cols else "NULL AS expired"
                )
            )
        )
        select_fields = [
        ...
        for content in contents:
            ...
            if expired:
                effective_end_time = self._parse_expire_date(expired, f"content.expired(id={old_id})")
                if effective_end_time:
                    print(f"  内容 {old_id}: 解析过期日期 '{expired}' -> {effective_end_time}")
                else:
                    print(f"  内容 {old_id}: 过期日期解析失败 '{expired}'")
